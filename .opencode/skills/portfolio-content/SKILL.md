...
- `EXPERIENCE` — newest first. Each entry: unique sequential `rev` (oldest role = `REV.A`), `role`, `company`, `period` (full), `span` (short, for the timeline spine), `place`, `summary`, `points[]`, `tags[]`, `current: true` if ongoing.
